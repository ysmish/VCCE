# =============================
# Database Models for Collaborative C Code Editor
# =============================
# This file defines all SQLAlchemy models and relationships for users, projects, documents, exercises, and chat.
...
# =============================
# Many-to-many relationship between users and projects, with permissions.
...
class User(db.Model):
    """
    User account for authentication and project ownership/collaboration.
    """
...
class Project(db.Model):
    """
    A collaborative coding project, owned by a user and may have collaborators.
    """
    id = db.Column(db.Integer, primary_key=True)
...
class Document(db.Model):
    """
    A document (file) within a project, owned by a user.
    """
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(100), nullable=False)
    content = db.Column(db.Text, nullable=True)
...
class Exercise(db.Model):
    """
    A coding exercise with test cases and solution.
    """
    id = db.Column(db.Integer, primary_key=True)
...
class CompilationHistory(db.Model):
    """
    History of code compilations and executions for auditing and feedback.
    """
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)
...
class ChatMessage(db.Model):
    """
    A chat message sent in a project room.
    """
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)
    project_id = db.Column(db.Integer, db.ForeignKey('project.id'), nullable=False)
